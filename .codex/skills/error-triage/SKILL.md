---
name: error-triage
description: "Diagnose a concrete pasted failure from logs, stack traces, test output, or build output. Use only when the user explicitly invokes $error-triage or a supported slash form; establish an evidence-backed root cause and propose or apply a narrow safe fix."
---
...
Run only after explicit `$error-triage` invocation (or `/error-triage` where supported). Start with the error text and failing command. Use expected versus actual behavior, reproduction steps, runtime/version, and recent changes when they are available. Ask one focused question only when a missing detail blocks a responsible diagnosis.
...
1. Read applicable `AGENTS.md` and any repo-local inspection guidance. Follow the repository's command, test, data, and approval rules.
...
- Do not create an artifact by default. Save one only when the user asks or repository rules require it. Use `.codex/artifacts/error-triage/YYYY-MM-DD_HHmm_<topic>.md` and record only confirmed, redacted facts.
