---
name: execute
description: "Implement a user-approved, scoped repository task or named plan. Use only when the user explicitly invokes $execute or a supported slash form; inspect relevant code, make minimal changes, self-review the changes, and run focused verification."
---
...
Run only after explicit `$execute` invocation (or `/execute` where supported). A direct request to implement a clearly described task is approval to implement it. A draft plan is not approval unless the user explicitly asks to execute that plan.
...
Do not commit, push, open a pull request, or write an execution artifact unless the user asks or repository instructions require it. When an artifact is requested, use `.codex/artifacts/execute/YYYY-MM-DD_HHmm_<topic>.md` and redact sensitive data.
