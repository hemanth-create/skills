---
name: project-start
description: "Bootstrap or refresh a repo-local Codex workspace from verified repository evidence. Use only after explicit $project-start or slash invocation; perform read-only discovery first and do not rewrite an existing AGENTS.md without user confirmation."
---

# Project Start

Use this skill to make a repository easier for future Codex work without treating onboarding as a code change. It is not for generic repository questions.

## Ownership and boundaries

Run only after explicit `$project-start` invocation, or its supported slash form. Read applicable repository instructions before writing anything.

- An existing `AGENTS.md` is repository-owned: preserve it unless the user explicitly asks to change a named section.
- If no `AGENTS.md` exists, create a minimal one only from verified repository facts and user-provided command boundaries. Mark unknowns rather than inventing commands, architecture, or deployment ownership.
- Create missing `.codex/README.md` and `.codex/status.md` from [the onboarding templates](references/onboarding_templates.md). Preserve useful existing content; update only clearly generated status facts.
...
5. Create or safely refresh the onboarding files allowed by the ownership rules, then save one artifact under `.codex/artifacts/project-start/YYYY-MM-DD_HHmm_<topic>.md`. Never overwrite an artifact; append `-2`, `-3`, and so on when needed.
