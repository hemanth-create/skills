---
name: sub-task
description: "Decompose a named plan into traceable, independently executable work units. Use only after explicit $sub-task or slash invocation with a readable plan path; do not re-plan or implement, and write an artifact only after the plan can be verified."
---
...
Run only after explicit `$sub-task` invocation, or its supported slash form. Require a readable plan path. If it is missing, unreadable, or outside the available workspace, ask for a valid path and do not create an artifact.
...
6. Save the valid breakdown under `.codex/artifacts/sub-task/YYYY-MM-DD_HHmm_<topic>.md`, using [the sub-task template](references/subtask_template.md). Never overwrite an artifact; append `-2`, `-3`, and so on when needed.
