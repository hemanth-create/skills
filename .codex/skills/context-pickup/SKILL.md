...
2. Resolve the source in this order: a user-supplied path, a named artifact type, then a narrow artifact search. For `latest handoff`, limit results to `.codex/artifacts/codex-context-handoff/`; for `latest <skill-name>`, limit results to `.codex/artifacts/<skill-name>/`. If that folder does not exist, say so rather than searching elsewhere.
