...
description: "Find evidence-backed unnecessary complexity in a named diff, file set, or implementation without applying fixes. Use only after explicit /ponytail-review invocation; preserve required behavior, safety, accessibility, compatibility, and tests."
---
...
Run only after explicit `/ponytail-review` invocation. Require a named diff, files, implementation, or plan plus its intended behavior. Ask one focused question if the target or expected behavior is missing.

Read applicable `CLAUDE.md` instructions and inspect only the selected scope plus the minimum context needed to verify a finding. Do not create artifacts, run broad checks, or invoke other skills automatically.
