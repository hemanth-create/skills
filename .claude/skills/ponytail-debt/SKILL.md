---
name: ponytail-debt
description: "Report deliberate, structured Ponytail shortcuts in a named repository scope without changing files. Use only after explicit /ponytail-debt invocation; distinguish complete markers from incomplete or legacy markers instead of inventing missing context."
---
...
1. Run only after explicit `/ponytail-debt` invocation. Use the named scope or the current repository root if none is specified.
2. Read applicable `CLAUDE.md` instructions. Use repository-approved, read-only search tools; exclude generated, vendored, cache, dependency, virtual-environment, and `.git` paths.
