---
name: multi-review
description: "Reconcile two independent read-only review lenses for a named implementation, plan, or subtask. Use only after explicit /multi-review invocation; produce one evidence-backed verdict without editing or broad exploration."
---
...
Run only after explicit `/multi-review` invocation. Require:

- a named target (files, diff, plan, task, or prior-agent output); and
- the expected behavior, approved plan, or acceptance criteria.

If either is missing, ask one focused question before reviewing. Read applicable `CLAUDE.md` instructions and preserve unrelated working-tree changes.
