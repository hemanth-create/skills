...
description: Create a fact-bounded Markdown handoff so a fresh Claude Code task can continue a repository workflow. Use only when the user explicitly invokes @claude-context-handoff or /claude-context-handoff to preserve confirmed state, decisions, validation, and exact next steps; do not use for a generic summary.
---
...
5. Write the handoff to `.claude/artifacts/claude-context-handoff/YYYY-MM-DD_HHmm_<topic>.md`. If that name already exists, append `-2`, `-3`, and so on.
6. Update an existing handoff's `updated` field rather than overwriting its original `created` value.
7. Maintain an artifact index only when one already exists or the user asks for it. Do not change `CLAUDE.md`, runtime code, deployment files, schemas, or CI/CD while creating a handoff.
