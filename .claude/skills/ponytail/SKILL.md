---
name: ponytail
description: "Choose and implement the smallest correct, evidence-backed change for a named coding task. Use only after explicit /ponytail invocation; preserve explicit requirements, safety, accessibility, and supported-platform behavior rather than simplifying them away."
---
...
Require a named outcome and acceptance criteria. Read applicable `CLAUDE.md` instructions, the relevant implementation, and nearby call paths before editing. Ask one focused question when an unresolved trade-off would change the design.
