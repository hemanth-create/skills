...
Use `/agent-relay` in place of `@agent-relay` only when the environment supports slash-style invocation. Use `claude` as the current participant name unless the user explicitly supplies `--as NAME`.
...
6. Append exactly one block using the next sequence number:
   ...
   Replace `claude` with the explicit `--as` name when supplied.
