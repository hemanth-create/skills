...
description: "Create an evidence-backed, multi-file product sign-off bundle for a named product, feature, or release. Use only after explicit /product-signoff invocation; inspect accessible context and supplied chat or handoff material, distinguish evidence from gaps, and never claim an unavailable chat or human approval was reviewed."
---
...
Run only after explicit `/product-signoff` invocation. Require a named product, feature, release, or sign-off decision. Ask one focused question if the target or decision owner is materially unclear.

Choose the requested mode:

- **Full sign-off bundle (default):** create a directory under `.claude/artifacts/product-signoff/YYYY-MM-DD_HHmm_<topic>/`.
...
Read applicable `CLAUDE.md` instructions, then inspect only sources relevant to the decision. Prefer evidence in this order:
